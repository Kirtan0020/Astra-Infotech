...
export default function TechMarquee() {
  return (
    <div className="relative h-72 overflow-hidden bg-[var(--color-bg)] md:h-72">
...
function MarqueeRibbon({ rotate, rotateMd, reverse = false, tint }) {
  return (
    ...
    >
      <div
        className={`overflow-hidden bg-gradient-to-r ${tint} to-transparent bg-[var(--color-text)]/[0.03] py-4 shadow-[0_10px_40px_-15px_rgba(0,0,0,0.6)] backdrop-blur-sm md:py-6`}
      >
        <div
        ...
        >
          {items.map((item, i) => (
            ...
            >
              ...
              <span className="text-sm font-medium md:text-lg">{item.label}</span>
            </div>
          ))}
        </div>
      </div>
